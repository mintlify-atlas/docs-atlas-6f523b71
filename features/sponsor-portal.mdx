...
<Note>
  The `ProtectedRoute` component verifies that `role` satisfies `role.type === 'sponsor'` before granting access to `/sponsor`. Any visitor without a valid sponsor session is redirected to `/login`.
</Note>
...
`ProtectedRoute` checks `role.type === 'sponsor'` (or `role === 'sponsor'` string). Any user with a different role is redirected to `/auth/Home`.
