...
[...eboardEntries]
  ...
  .map((entry, index) => <MemberCard key={index} entry={entry} />)
```
...
<CardGroup cols={2}>
  ...
  <Card title="Name" icon="identification">
    Displays `entry.name` or a dash (`"—"`) if `name` is `null`. This handles in-progress board transitions where a seat is defined but not yet filled.
  </Card>
