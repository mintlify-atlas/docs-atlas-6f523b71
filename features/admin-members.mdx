...
description: "Full reference for the three member panels in the Admin Dashboard: active general members, archived (soft-deleted) members, and read-only alumni, including rank and archive/restore flows."
---
...
<Steps>
  ...
  <Step title="Admin.tsx fetches full MemberDetail">
    If the email is not already in the `memberDetails` cache, `GET /member-info/:email` is called. The raw API response is normalized into a `MemberDetail` object:
    ```ts
...
`Admin.tsx` uses a ref-stored callback pattern to allow `ArchivedMembersList` to expose its internal `fetchArchivedMembers` function to the parent:
