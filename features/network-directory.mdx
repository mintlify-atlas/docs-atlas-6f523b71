...
EboardPage has **no** graduation year, major, or status filters — the filter panel is empty (`availableGraduationYears: []`, etc.) and only fuzzy name/email/major/about search is available.
...
<Tabs>
  ...
  <Tab title="AlumniPage sort options">
    Same as NetworkingPage minus `rank-asc` and `rank-desc` (all entries are alumni). Default: `name-asc`.
  </Tab>
