...
### `GET /sponsors/:name/details`

Retrieves detailed profile information for a specific sponsor.

**Auth:** `Authorization: Bearer <token>` (required)

**Path param:** `name` — URL-encoded company name.
...
```bash
curl -X GET "https://asubap-backend.vercel.app/sponsors/Acme%20Corp/details" \
  -H "Authorization: Bearer <token>"
```
